...
export default function DriverProfile() {
  ...
  const fetchDriverDetails = async () => {
    ...
    try {
      setLoading(true)
      // Use user_id to fetch driver details
  ...
  const handleVehicleUpdate = async () => {
    ...
    try {
      if (driverDetails) {
        ...
        // Create new driver record using user_id as primary key
        const { error } = await supabase.from("drivers").insert({
          id: profile.user_id, // Use user_id as the primary key
          license_number: vehicleForm.licenseNumber || "",
          vehicle_plate: vehicleForm.vehiclePlate || "",
          vehicle_model: vehicleForm.vehicleModel || "",
          is_active: false, // Start as inactive until admin approves
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          // First ensure driver record exists, then update location
          if (driverDetails) {
            // Update existing driver record using user_id
            const { error } = await supabase
              .from("drivers")
            ...
            // Create driver record first if it doesn't exist using user_id
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Vehicle registration status</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Last Location Update</CardTitle>
              <MapPin className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {driverDetails?.last_location_update ? (
                  <span className="text-sm">{new Date(driverDetails.last_location_update).toLocaleDateString()}</span>
                ) : (
                  "Never"
                )}
              </div>
          ...
          <TabsContent value="vehicle" className="mt-4">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button onClick={handleVehicleUpdate} disabled={saving}>
